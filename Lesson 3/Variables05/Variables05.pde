...
void draw ()
{
  ...
  if(posY>(height+ballSize)) 
  {
    posY=0;
  }
     
  // direction changes with mouse position 
  if(mouseX<width/2) 
  {
    velX=random(map(mouseX,0,width,-10,0),2);
  }
  if(mouseX>=width/2) 
  {
    velX=random(-2,map(mouseX,0,width,0,10));
  }
  
  if(mouseY<height/2) 
  {
    velY=random(map(mouseY,0,height,-10,0),2);
  }
  if(mouseY>=height/2)
  {
    velY=random(-2,map(mouseY,0,height,0,10));
  }

  //update x and y values to position the ellipse
  posX=posX+velX;
  posY=posY+velY;
...
void keyReleased()
{  
  // If Key is released s or S, print a frames
  if (key == 's' || key == 'S')
  {  
  saveFrame("save-###.jpg"); 
  }

  //If Key is released is b or B, paint background
  if (key == 'B' || key == 'b')
  {
    back=!back;
  }
